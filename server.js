...
// 链接SMTP服务器
//const BoTIP = "http://127.0.0.1";
const BoTIP = "http://localhost";
var transporter = nodemailer.createTransport({
    ...
    port:465,
    auth:{
        user:'ymq010222@163.com',//邮箱
        pass:'DQQPGSAPCQUSOMTG'//邮箱密码或授权码
    }
});
var mailOptions = {};
async function initEmail(){
    mailOptions = {
        from:'"YuMengqi" <ymq010222@163.com>',//发送者
        to:await getEmail(),//接受者
 ...
 * 监听修改邮箱
 */
async function listenChangeEmail() {
    return new Promise(function(result){
        contract.events.SetEmail({},function(err,res){
            if(err){
                console.log("watch err",err);
                result(false);
            }else{
                mailOptions.to = res["returnValues"]["email"];
                console.log("Change Email: ",mailOptions.to);
                result(true);
            }
        });
    });
 ...
 * 监听测试事件
 */
async function listenAddTestEvent() {
    return new Promise(function(result){
        contract.events.AddEventTest({},function(err,res){
            if(err){
                console.log("watch err",err);
                result(false);
            }else{
                console.log("Get add test event and check sending email!");
                sendMail(res["returnValues"][1],res["returnValues"][0]);
                result(true);
            }
        });
    });
 ...
 * 发送邮件
 */
async function sendMail(request_id,device_account=undefined){
    // 检查是否订阅发邮件服务
    if(await getEmailServe()==false) {
        console.log("Dont't need to send");
        return;
    }

    // 获取申请名称
    var request_name = await getEventsClassInfo(request_id);
    if(request_name["class"]!=3) {
        console.log("Dont't need to send");
        return;
    }
    if(request_name==false) return false;
    request_name = request_name["name"];
    
    // 获取设备名称
    if(device_account!=undefined){
        var device_name = await getDeviceInfo(device_account);
        if(device_name==false){
            return false;
        }
        device_name = device_name["name"];
    }else{
        var device_name = "New Device";
    }
    ...
    transporter.sendMail(mailOptions,function(err,info){
        if(err){
            console.log("Error: Fail to send email: ",err);
            return;
        }
        console.log("发送成功");
    });
}
async function sendMailTest(request_name,device_name){
    if(await getEmailServe()==false) {
        console.log("Dont't need to send email");
        return;
    }
...
//sendMailTest("Open Light","Smart Light");

/* For test send email
const _getEmailServe = async () => {
    if(await getEmailServe()!=false) sendMail();
    else{console.log("Dont't need to send");}
}
...
async function getEmail(){
    var account = await getAccount0();
    return new Promise(function(result){
        if(account==false)result(false);
        contract.methods.getEmail().call({from:account})
        .then(function(res){
            console.log("Get Email: ",res);
            result(res);
        }, function(reason) {
            console.log("Error: Fail to Get Email(C)",reason);
            result(false);
        });
    });
}
...
async function web3Operation(commandString_){
    ...
    if("v" in command)return false;
    if("-fvbane" in command)return false;
    ...
    if(command.type=="ping"){
        ...
        */
        ...
        if(await authAddEvent(command.name)==false){
            ...
            if(add_type==false) return {'json':{'valid':false}};
            // 发邮件(函数中内置各种判断)
            sendMail(eventsClassInfo["id"],command.account);
            // 添加事件
